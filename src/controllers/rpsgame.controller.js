...
class RPSGAMEController {
	getLog = async(req, res, next) => {
		try{
			const result = await RPSGAMEService.getLog()
			res.send(result)
		}catch(err){
			next(err)
		}
	...
	getAvailable = async(req, res, next)=> {
		try{
			const result = await RPSGAMEService.getAvailable(req.query)
			res.send(result)
		}catch(err){
			next(err)
		}
	...
	getInvitation = async(req, res, next)=> {
		try{
			const result = await RPSGAMEService.getInvitation(req.query)
			res.send(result)
		}catch(err){
			next(err)
		}
